...
#define NUM_LEDS 30     //Replace this value with the numbers of leds on your strip. Note, some strips count 3 leds as a single segment, normally the ones operation at 12v
...
const String commandList[] = {"Off", "Fixed Color", "Color Fade", "Color Breathe", "Color Chase", "Color Chase Bounce", "Color Cycle", 
                        ...
                        "Rainbow Chase Bounce", "Color Gradient", "Fill", "Rainbow Fill", "Fill Gradient", "Custom Pattern", "Pixel"};
...
void FillGradient(){
  fill_gradient_RGB(leds, NUM_LEDS, foreColor, backColor);
  for(int i = pix; i < NUM_LEDS; i++){
    leds[i] = CRGB(0,0,0);
  }
  FastLED.show();
}
